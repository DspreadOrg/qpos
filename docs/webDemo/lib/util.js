...
function print(info){
   ...
   //   	 var startPos = log.selectionStart;
		 // var endPos = log.selectionEnd;
 
		 // // 保存滚动条
		 // var restoreTop = log.scrollTop;
		 // log.value = log.value.substring(0, startPos) + info + log.value.substring(endPos, log.value.length);
		 ...
		 // // log.focus();
		 // log.selectionStart = startPos + info.length;
		 // log.selectionEnd = startPos + info.length;
