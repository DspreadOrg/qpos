...
**1. Update IPEK key by TR-31 (recommend)**

The below keyBlock parameter is TR31 data block(support B and D version) which generated by HSM. Also, you can generate it by the [**TR31 Block Online Tool**](https://kabc.ca/keys/tr31) for test.
By default, QPOS is injected KBPK in factory. default KBPK: 0123456789ABCDEFFEDCBA9876543210. KSN is being considered to be placed in the optional block, and the format is as follows.
- Identifier: "KS"
- Length: "18"
...
**3. Update IPEK key by RSA (Asymetrical)**
...
?>HSM should use same KBPK as inside terminal to encrypt session keys to get TR-31 block, you can also generate it by the [**TR31 Block Online Tool**](https://kabc.ca/keys/tr31) for test, then call API pos.updateKeyByTR_31() to inject into terminal.

![](./_images/UpdateWorkKeyByTR31.png)
```java
//call the below api to inject ipek, let keyIndex = 0
pos.updateKeyByTR_31(keyIndex,keyBlock);
//callback
...
**3. Update master key by TR-31 (recommend)**
